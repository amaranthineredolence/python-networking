...
def run_script():
    ...
    ssh_key_path = ssh_key_entry.get()  # New line to get SSH key path
    ...
    for switch in list_of_switches:
        network_device = {
            ...
            "username": user,
            "password": password,
            "ssh_key_file": ssh_key_path,  # New line to add SSH key
        }
...
ssh_key_label = tk.Label(root, text="Enter the path to the SSH key file:")
ssh_key_label.pack()
ssh_key_entry = tk.Entry(root)
ssh_key_entry.pack()
ssh_key_button = tk.Button(root, text="Browse", command=lambda: ssh_key_entry.insert(tk.END, filedialog.askopenfilename()))
ssh_key_button.pack()
