    ...
    try:
        ...
        if commands:
            for command in commands:
                ...
                if errors:
                    logging.error(f"Error executing command '{command}' on {ip}:\n{errors}")
                else:
        ...
        logging.error(f"Authentication failed for device {ip}. Check credentials.")
    except paramiko.SSHException as e:
        logging.error(f"SSH connection error for device {ip}: {str(e)}")
    ...
    def start_configuration(self):
        try:
            ...
            for ip in device_ips:
                configure_device(ip, username, password=password, private_key=private_key_path, commands=commands)
        except Exception as e:
            logging.error(f"Error starting configuration: {str(e)}")
            print(f"Error starting configuration: {str(e)}")
