...
def run_script():
    ...
    ssh_key_location = ssh_key_location_entry.get()
    ...
    for switch in list_of_switches:
        network_device = {
            ...
            "username": user,
            "password": password,
            "key_file": ssh_key_location,  # Add the key_file parameter
        }
...
# SSH Key Location
ssh_key_location_label = tk.Label(root, text="Enter the path to the SSH key file:")
ssh_key_location_label.pack()
ssh_key_location_entry = tk.Entry(root)
ssh_key_location_entry.pack()
ssh_key_location_button = tk.Button(root, text="Browse", command=lambda: ssh_key_location_entry.insert(tk.END, filedialog.askopenfilename()))
ssh_key_location_button.pack()
