...
def run_script():
    ...
    ssh_key_path = ssh_key_path_entry.get()  # Get SSH key path
    ...
    for switch in list_of_switches:
        network_device = {
            ...
            "username": user,
            "password": password,
            "key_file": ssh_key_path,  # Add SSH key path
        }
...
# SSH Key path
ssh_key_path_label = tk.Label(root, text="Enter the path to the SSH key:")
ssh_key_path_label.pack()
ssh_key_path_entry = tk.Entry(root)
ssh_key_path_entry.pack()
ssh_key_path_button = tk.Button(root, text="Browse", command=lambda: ssh_key_path_entry.insert(tk.END, filedialog.askopenfilename()))
ssh_key_path_button.pack()
