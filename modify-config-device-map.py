...
for switch in list_of_switches:
    ...
    try:
        ...
        # Manually map the device_type if necessary
        device_type_map = {
            'cisco_ios': 'cisco_ios',
            'cisco_xe': 'cisco_ios',  # You can add more mappings as needed
            'cisco_asa': 'cisco_ios',  # Example: 'cisco_asa' maps to 'cisco_ios'
        }

        # Update the network_device dictionary with the detected device_type
        network_device["device_type"] = device_type_map.get(device_type, 'autodetect')
